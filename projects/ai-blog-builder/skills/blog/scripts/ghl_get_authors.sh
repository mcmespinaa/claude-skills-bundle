#!/usr/bin/env bash
# ghl_get_authors.sh — List blog authors for a GHL location
# Usage: ./ghl_get_authors.sh [--location ces]
...
echo "Fetching authors for location: $GHL_LOCATION_ID" >&2

curl -s -X GET "$API_BASE/blogs/authors?locationId=$GHL_LOCATION_ID&limit=10&offset=0" \
