...
JSON structures used by skill-builder for evals, grading, benchmarks, and comparisons.
...
- `evals[].expectations`: Verifiable statements to assert against outputs
...
```json
{
  ...
  "assertions": [
    ...
    "The response includes error handling"
  ]
}
```
...
**Important:** The viewer expects fields `text`, `passed`, and `evidence` in expectations. Using other field names will break the viewer.
...
**How to capture:** When a subagent task completes, the notification includes `total_tokens` and `duration_ms`. Save immediately — not persisted elsewhere.
...
```json
{
  ...
  "total_tool_calls": 18,
  "total_steps": 6,
  "files_created": ["output.pdf", "data.json"],
  "errors_encountered": 0,
  "output_chars": 12450,
  "transcript_chars": 3200
}
...
```json
{
  ...
  "notes": [
    "Analyst observations go here"
  ]
}
```

**Important:** The viewer reads field names exactly. `configuration` must be `"with_skill"` or `"without_skill"`. `pass_rate` must be nested under `result`. Using different field names causes the viewer to show zeros.
...
User feedback from the eval viewer. Located at `<workspace>/iteration-N/feedback.json`.
